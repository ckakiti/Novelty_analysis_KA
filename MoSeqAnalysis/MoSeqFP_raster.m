...
% basefolder = '/media/alex/DataDrive1/MoSeqData/ATLA/ATLA_MoSeq';
basefolder = '/home/alex/Programs/DeepLabCut_new/DeepLabCut/videos/Explorers_DLC/';
whichMouse = 'Aconcagua';
whichDate  = '210613';
whichSession = 'N1'; %Hab2 %N1 %R1(arm+delivery) %R2(consumption)
%whichFile = 1;

% load file w/ significant events (e.g. pokes, approaches, etc)
cd(basefolder)
load('PokesApproaches_7cm_30min.mat')
whichMouse_idx = find(strcmp({Mice.name},whichMouse));


cd(fullfile(basefolder, whichMouse, whichDate))

% filelist = dir('session*');
% cd(filelist(whichFile).name)

depthts_file = dir('*depth_ts.txt');
filename = depthts_file(1).name; %'depth_ts.txt';
...
filename = rgbts_file(1).name; %'rgb_ts.txt';
fileID = fopen(filename,'r');
dataArray = textscan(fileID, formatSpec, 'Delimiter', delimiter, 'MultipleDelimsAsOne', true, ...
    'TextType', 'string', 'EmptyValue', NaN,  'ReturnOnError', false);
fclose(fileID);
...
%% (2) %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
FP_file = dir('*FP.mat');      % from FPdat_import.m
rgb_FP_file = dir('*_rgb_ts'); %  "               "
%load('MoSeqFP.mat');          % from FPDataTransfer.py
load(FP_file(1).name)
rgb_FP = load(rgb_FP_file(1).name);
%LEDtime=csvread('LED_on');    % from KW_1_LED.m

% load indices for significant events (e.g. poke, boutStart, etc)
%      typically generated by bout_analysis.m or recorded by hand
if(strcmp(whichSession,'N1'))
    %boutFile = dir('*boutStart'); %nose
    %pokeFile = dir('*poke');      %nose
    %boutFile = dir('poke*FP*');
    %pokeFile = dir('poke*FP*');
    pokeFile = Mice(whichMouse_idx).Pokes_Day3; % from bout_analysis.m
    boutFile = pokeFile;
...
% if there's a discrepancy in pokeFile (has indices outside of rgb_FP)
if(0)
    pokeFile(pokeFile<rgb_FP(1))=[];
end
...
%acttime=acttime+900; % when extracting, depth frames were trimmed by 900

% convert poke frame number (rgb, from bout_analysis.m) to FP frame number
actlen = length(acttime);
acttime_index  = zeros(actlen,1);
acttime2_index = zeros(actlen,1);
for poke_iter = 1:actlen
    curr_poke_idx = find(rgb_FP(:,1)==pokeFile(poke_iter));
    acttime_index(poke_iter,1)  = rgb_FP(curr_poke_idx,2);
    acttime2_index(poke_iter,1) = rgb_FP(curr_poke_idx,2);
end

if(0)
% indices for tone (already in FP frame number)
acttime_index  = TTL_on;
acttime2_index = TTL_on;

% convert LED_on frame number to FP frame number (see FPdat_import.m)
...
% include ts info for salient events (e.g. pokes, reward delivery)
%  (convert depthts indices to rgbts+FP indices)

% load poke_rgb, pos_within_TTL variables
% load rgbts, depthts (see convert_rgbts_depthts.m/bout_analysis.m)
% load event label file (csv generated by hand)
% convert depth frame from csv to rgb frame
% locate rgb frame within pos_within_TTL
% save new csv with depth/rgb/FP indices
...
for actiter=1:actlen
    acttime_index(actiter,1)=find(tstep>depthts(acttime(actiter)),1); %%%%%%%%%%%%%%%
    acttime2_index(actiter,1)=find(tstep>depthts(acttime2(actiter)),1); %%%%%%%%%%%%%%%
...
% GCaMP=ch00;
% tdTom=ch01;
GCaMP = cart_GCaMP; % from FPdat_import.m
tdTom = cart_tdTom; % from FPdat_import.m
...
plotind(plotind<=0)=1;
plotind(plotind>length(GCaMP))=length(GCaMP);
...
plotind2(plotind2>length(GCaMP))=length(GCaMP);
...
title_dFF_1     = 'GCaMP signal aligned to poke';
xlabel_dFF_1    = 'time of poke (ms)';
xlabel_raster_1 = 'time - poke (s)';

% average GCaMP activity aligned to event (poke, bout start, reward delivery, etc)
fig1 = figure(1);
set(gcf, 'Position', [500 500 800 518])
...
imagesc(b1, [-4 4])
colorbar
title(['GCaMP raster: ' whichMouse])
set(gca, 'XTick', 0:100:1000, 'XTickLabel', c, 'FontSize', 16)
xlabel(xlabel_raster_1)
ylabel('trial')
axis square
...
if(0)
    saveas(fig1, ['FP_' whichMouse '_' whichSession '.tif'])
    saveas(fig2, ['FP_raster_GCaMP_' whichMouse '_' whichSession '.tif'])
    saveas(fig3, ['FP_raster_tdTom_' whichMouse '_' whichSession '.tif'])
    close all
end
