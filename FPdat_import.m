...
% load "LED is on" data (detected by KW_1_LED.m)
LED_on_name = dir('LED_on');
load(LED_on_name.name)
LED_on_max = LED_on'; %LED_on_max %%%%%%%%%%%%%%

% load FP data (including TTL pulse ~= LED)
%   generated by custom program on lab computer
%file = 'imaging.dat';
file = 'FP_data'; %FP_data %tone_data
...
close all
hold on
p1 = plot(cart_GCaMP, 'g');
p2 = plot(cart_tdTom, 'r');
l1 = line([TTL_on TTL_on]', repmat([1 4.2]', 1, length(TTL_on)), 'color', 'k');
ylim([0.5 5])
...
save([currMouse, '_' currDate '_FP'], ...
   'cart_GCaMP', 'cart_tdTom', 'TTL', 'TTL_on', 'pos_within_TTL')
